...
import os, math, re, sys, fnmatch, string, json, get_twitter_data 
...
def processTweet(tweet): 
	# process the tweets #Convert to lower case 
	tweet = tweet.lower() 
	#Convert www.* or https?://* to URL 
	tweet = re.sub('((www\.[^\s]+)|(https?://[^\s]+))','URL',tweet) #Convert @username to AT_USER 
	tweet = re.sub('@[^\s]+','AT_USER',tweet) #Remove additional white spaces 
	tweet = re.sub('[\s]+', ' ', tweet) #Replace #word with word 
	tweet = re.sub(r'#([^\s]+)', r'\1', tweet) #trim 
	tweet = tweet.strip('\'"') 
	
	return re.sub(r'[^\x00-\x7F]+',' ', tweet) #end
...
class wxToolTipExample(object):
    def __init__(self):
        ...
        self.dataX = x
        self.dataY = y
        self.axis.plot(self.dataX, self.dataY, linestyle='-', marker='o', markersize=10, label='plot')
...
if __name__ == '__main__':
    for i in range(0, 15):
		...
		for a in tweets[sentence]:
			processed = processTweet(a)
			print processed
			twee.append(processed)
			x.append(ctr)
			ctr = ctr + 1
			print "\n"
			ss = sentiment(processed)
			#print str(ss)
			#print ss['compound']
			y.append(ss['compound'])
			print "\n"
	    '''
	    for sentence in tweets:
		for a in tweets[sentence]:
